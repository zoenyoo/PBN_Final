...
We sought to build upon existing research on 311 reporting. Significant research has been done on 311 reporting in Boston, using a mixed methods approach that is equal parts social science and anthropology. O'Brien et. al. found that “higher attachment with the space was associated with a greater motivation to enforce local social norms and benefit the broader community.” [1] With this framework in place, the use of mobility-related 311 cases in comparison to actual traffic injuries allowed us to focus in on places not only where reporting is low, but where reporting does not reflect actual conditions. 
...
![Intersection Polygons](./pics/ip_map.jpg){#fig:ip_map}
...
dat1 %>% summarise(across(where(is.numeric), ~ mean(.x, na.rm = TRUE))) %>% kable(caption="High Mobility Cases in the HIN") %>%
  kable_styling("striped", full_width = F) %>% scroll_box(width = "1000px", height = "150px")
dat2 %>% summarise(across(where(is.numeric), ~ mean(.x, na.rm = TRUE))) %>% kable(caption="Low Mobility Cases in the HIN") %>%
  kable_styling("striped", full_width = F) %>% scroll_box(width = "1000px", height = "150px")
dat3 %>% summarise(across(where(is.numeric), ~ mean(.x, na.rm = TRUE))) %>% kable(caption="High Mobility Cases NOT in the HIN") %>%
  kable_styling("striped", full_width = F) %>% scroll_box(width = "1000px", height = "150px")
...
As we can see from the correlation plot, there are m
...
These are striking results, and have interesting implications in relationship to existing literature on 311 reporting. O'Brien et. al., using a mixed methods approach, say that "this paints a picture of custodianship, at least as measured through CRM (311) reports, as a pro-social act intended to provide benefit to oneself and others" and that "The effectiveness of the CRM (311) system in promoting neighborhood maintenance may rely primarily on goodwill for the neighborhood". [1] With this in mind, we see a negative association between driving to commute, and poverty rates, on pro-social behaviors like custodianship and neighborhood goodwill, as well as a positive association between home ownership and these behaviors. There is already a wealth of research proving the benefits of home ownership [2] and the negative implications of driving [3], while the negative implications of poverty are intuitive. However, in utilizing 311 data and ACS data, we can begin to understand the relationship between civic participation and more closely studied factors such as commute mode choice, housing, and poverty. 
...
[1] O'Brien, Daniel Tumminelli, Eric Gordon, and Jessica Baldwin. “Caring about the Community, Counteracting Disorder: 311 Reports of Public Issues as Expressions of Territoriality.” Journal of Environmental Psychology 40 (2014): 320–30. https://doi.org/10.1016/j.jenvp.2014.08.003. 
[2] Coulson, N. Edward, and Herman Li. "Measuring the external benefits of homeownership." Journal of Urban Economics 77 (2013): 57-67.
[3] Conkle, Ann, and Catherine West. "Psychology on the Road." APS Observer 21, no. 6 (2008).
