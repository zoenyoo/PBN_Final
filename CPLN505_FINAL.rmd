...
ip2 <- ip %>% 
  mutate(all311 = lengths(st_intersects(.,all311)),
         ...
         hin = as.numeric(if_else(hin_c > 0, "1", "0")),
         ...
         inj = as.numeric(if_else(lengths(st_intersects(.,crash_injury)) > 0, "1", "0")),
...
dat <- dat %>% 
  mutate(hm311 = as.numeric(if_else(m311 > 3, "1", "0")),
         logm311 = log(m311))
...
ggplot(data=dat, aes(x=all311,y=m311))+
  geom_point()
...
hinMod1 <- glm(hin ~ crsh_c + inj_c + PCT_own + all311 + m311,
               data=dat %>% st_drop_geometry(),
               family="binomial" (link="logit"))


hinMod2 <- glm(hin ~ crsh_c + inj_c + PCT_own,
               data=dat %>% st_drop_geometry(),
               family="binomial" (link="logit"))

stargazer(hinMod1, hinMod2,
          type="html", title="Agricultural/pasture/forest land-to-urban land use change",   
          column.labels = c("Model 1", "Model 2"),
          single.row = TRUE, digits=2)

```

## High Mobility 311 Case Intersections

```{r high_311_bl}

# high311Mod1 <- glm(hm311 ~ ,
#                data=dat %>% st_drop_geometry(),
#                family="binomial" (link="logit"))
